...
async function apiRaw(path, options = {}) {
    ...
    if (res.status === 401 && path !== "/crm/api/auth/login") {
        logout();
...
async function doLogin(e) {
    ...
    try {
        const data = await apiRaw("/crm/api/auth/login", {
            method: "POST",
            body: JSON.stringify({
...
async function searchUsers(q) {
    try {
        const headers = {};
        if (token) headers["Authorization"] = "Bearer " + token;
        const res = await fetch("/crm/api/segments/users?q=" + encodeURIComponent(q), { headers });
...
async function openUserList() {
    ...
    try {
        const headers = {};
        if (token) headers["Authorization"] = "Bearer " + token;
        const res = await fetch("/crm/api/segments/users", { headers });
        if (res.status === 401) { logout(); return; }
        const data = await res.json().catch(() => ({}));
...
async function openUserDetails(id) {
    ...
    try {
        const headers = {};
        if (token) headers["Authorization"] = "Bearer " + token;
        const res = await fetch("/crm/api/segments/users/" + encodeURIComponent(id), { headers });
        if (res.status === 401) { logout(); return; }
        const data = await res.json().catch(() => ({}));
...
async function doPreview() {
    ...
    btn.disabled = true;
    try {
        const data = await apiRaw("/crm/api/segments/preview", {
            method: "POST",
...
async function doSend() {
    ...
    try {
        const headers = {};
        if (token) headers["Authorization"] = "Bearer " + token;
        const res = await fetch("/crm/api/broadcast", { method: "POST", headers, body: fd });
        const data = await res.json().catch(() => ({}));
...
function startPolling(id) {
    ...
    const tick = async () => {
        try {
            const s = await apiRaw("/crm/api/broadcast/" + id + "/status");
